...
// Tailwind v4.0.0's Vite plugin assumes compiler.globs is always an array.
// The installed @tailwindcss/node returns undefined for plain/reference-only CSS,
// which crashes build on Astro style blocks and standalone CSS modules.
const tailwindViteEntry = path.join(
	fs.realpathSync(path.resolve("./node_modules/@tailwindcss/vite")),
	"dist/index.mjs",
);
const tailwindViteSource = fs.readFileSync(tailwindViteEntry, "utf8");
if (tailwindViteSource.includes(".concat(this.compiler.globs)")) {
	fs.writeFileSync(
		tailwindViteEntry,
		tailwindViteSource.replace(
