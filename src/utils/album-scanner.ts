...
async function processAlbumFolder(
	...
	if (isExternalMode) {
		...
		cover = hasWebpCover
			? withBase(`/images/albums/${folderName}/cover.webp`)
			: withBase(`/images/albums/${folderName}/cover.jpg`);
...
function scanPhotos(folderPath: string, albumId: string): Photo[] {
	...
	imageFiles.forEach((file, index) => {
		...
		const src = fileWebpMap.has(file)
			? withBase(`/images/albums/${albumId}/${fileWebpMap.get(file)}`)
			: withBase(`/images/albums/${albumId}/${file}`);
